...
#pragma mark -  Bar Menu for Left and Right
    ...
    else
	{
      ...
      NSString *EmployeeUserID = [[NSUserDefaults standardUserDefaults]stringForKey:@"EmployeeUserId"];
        ...
        if([[[NSUserDefaults standardUserDefaults] objectForKey:@"Update"]isEqualToString:@"Login"])
        {
            ...
            registerData= [[NSUserDefaults standardUserDefaults] objectForKey:@"employerRegister"];
            if (registerData!=nil) {
        ...
        if (EmployeeUserID==nil) {
            ...
            if (kAppDel.EmployerProfileImage==nil) {
                 [_profileImage setImage:[UIImage imageNamed:@"profile"] forState:UIControlStateNormal];
            }else{
            [_profileImage setImage:kAppDel.EmployerProfileImage forState:UIControlStateNormal];
            }
        }
        else{
            ...
            if (kAppDel.EmployeeProfileImage==nil) {
                 [_profileImage setImage:[UIImage imageNamed:@"plceholder"] forState:UIControlStateNormal];
            }
            else{
            [_profileImage setImage:kAppDel.EmployeeProfileImage forState:UIControlStateNormal];
            }
        }
        ...
        _profileBadge.frame = CGRectMake(_profileImage.frame.size.width+_profileImage.frame.origin.x-13, _profileImage.frame.origin.y+5, 20, 20);
                
        UIImageView *logo = [[UIImageView alloc]initWithFrame:CGRectMake(60, 4, 120, 28)];
                
        logo.image = [UIImage imageNamed:@"peoplenect_logo_.png"];
					...
					animated:(BOOL)animated
{
    ...
    {
        if ([viewController isKindOfClass:[ employeeJobNotification class]])
        {
            viewController.navigationItem.leftBarButtonItem = [self barButtonItemForMenu:MenuLeft];
        }
        if ([viewController isKindOfClass:[ MenuCtr class]])
        {
            viewController.navigationItem.leftBarButtonItem = [self barButtonItemForMenu:MenuLeft];
        }
       
        
   else  if ([viewController isKindOfClass:[EmployeeSettings class]])
    {
        viewController.navigationItem.leftBarButtonItem = [GlobalMethods customNavigationBarButton:@selector(leftMenuSelected:) Target:self Image:@"Gray_right_arrow_" ];
    }
        else if ([viewController isKindOfClass:[onGoingJobs class]])
        {
            viewController.navigationItem.leftBarButtonItem = [GlobalMethods customNavigationBarButton:@selector(leftMenuSelected:) Target:self Image:@"Gray_right_arrow_" ];
        }
        
   else  if ([viewController isKindOfClass:[employeePendingInvitation class]])
   {
       
       viewController.navigationItem.leftBarButtonItem = [GlobalMethods customNavigationBarButton:@selector(leftMenuSelected:) Target:self Image:@"Gray_right_arrow_" ];
   }
        
     if ([viewController isKindOfClass:[employeeJobHistory class]])
   {
       viewController.navigationItem.leftBarButtonItem = [GlobalMethods customNavigationBarButton:@selector(leftMenuSelected:) Target:self Image:@"Gray_right_arrow_" ];
   }
        if ([viewController isKindOfClass:[openJob class]]) {
            viewController.navigationItem.leftBarButtonItem = [GlobalMethods customNavigationBarButton:@selector(leftMenuSelected:) Target:self Image:@"Gray_right_arrow_" ];
        }
     
        if ([viewController isKindOfClass:[employeeRevenues class]])
        {
            viewController.navigationItem.leftBarButtonItem = [GlobalMethods customNavigationBarButton:@selector(leftMenuSelected:) Target:self Image:@"Gray_right_arrow_"];
        }
        if ([viewController isKindOfClass:[employerTransaction class] ])
        {
             viewController.navigationItem.leftBarButtonItem = [GlobalMethods customNavigationBarButton:@selector(leftMenuSelected:) Target:self Image:@"Gray_right_arrow_"];
        }
        
        if ([viewController isKindOfClass:[employerSettings class]]) {
            viewController.navigationItem.leftBarButtonItem = [GlobalMethods customNavigationBarButton:@selector(leftMenuSelected:) Target:self Image:@"Gray_right_arrow_"];
        }
        if ([viewController isKindOfClass:[CategoryEmployeeCtr class]]) {
             viewController.navigationItem.leftBarButtonItem = [GlobalMethods customNavigationBarButton:@selector(leftMenuSelected:) Target:self Image:@"Gray_right_arrow_"];
        }
        if ([viewController isKindOfClass:[EmployerJobHistory class]]) {
            viewController.navigationItem.leftBarButtonItem = [GlobalMethods customNavigationBarButton:@selector(leftMenuSelected:) Target:self Image:@"Gray_right_arrow_"];
        }
...
- (void)righttMenuSelected:(id)sender
{
    ...
    NSLog(@"You Have clicked on Right Side");
